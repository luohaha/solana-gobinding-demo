...
/// this code wil be compiled to cdylibs, then called by go language
#[no_mangle]
pub unsafe extern  "C" fn test_call_by_go(keypair_path : *const libc::c_char) {
    ...
    let ret = get_balance_requirement(&connection);
    let balance_requirement = ret.unwrap();
    println!(
        "({}) lamports are required for this transaction.",
        balance_requirement
    );
    let player = utils::get_player().unwrap();
    let program = get_program(CStr::from_ptr(keypair_path).to_str().unwrap(), &connection).unwrap();

    create_greeting_account(&player, &program, &connection).unwrap();

    say_hello(&player, &program, &connection).unwrap();
    println!(
        "({}) greetings have been sent.",
        count_greetings(&player, &program, &connection).unwrap()
    );
}
